...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, allsprites, images):
        ...
        self._current_visual_key = ("normal", self.current_direction)
        self._last_size_score = self.size_score
    ...
    def update(self):
        ...
        if self.size_score != self._last_size_score:
            self._render_dirty = True
            self._last_size_score = self.size_score
    ...
    def _get_base_image(self):
        state_name, direction = self._current_visual_key
        if state_name == "munch":
            return self.munching_images[direction]
        if state_name == "gold":
            return self.get_gold_image()
        return self.original_images[direction]

    def _refresh_scaled_assets(self):
        ...
        cache_key = (self._current_visual_key, self.size_score, new_width, new_height)

        cached_assets = self._scaled_cache.get(cache_key)
        if cached_assets is None:
            scaled_image = pygame.transform.smoothscale(base_image, (new_width, new_height))
            face_mask_image = pygame.transform.smoothscale(
                self.images[f"player_{self.current_direction}_face"],
                (new_width, new_height),
            )
            body_mask_image = pygame.transform.smoothscale(
                self.images[f"player_{self.current_direction}"],
                (new_width, new_height),
            )
            cached_assets = {
                "image": scaled_image,
                "face_mask": pygame.mask.from_surface(face_mask_image),
                "body_mask": pygame.mask.from_surface(body_mask_image),
            }
            self._scaled_cache[cache_key] = cached_assets

        current_pos = getattr(self.rect, "topleft", (int(self.pos[0]), int(self.pos[1])))
        self.image = cached_assets["image"].copy()
        self.face_mask = cached_assets["face_mask"]
        self.body_mask = cached_assets["body_mask"]
    ...
    def handle_munching_animation(self):
        ...
        previous_key = self._current_visual_key
        if self.munching_timer % self.MUNCH_ANIMATION_SPEED < self.MUNCH_ANIMATION_SPEED // 2:
            self._current_visual_key = ("munch", self.current_direction)
        else:
            self._current_visual_key = ("normal", self.current_direction)
        if self._current_visual_key != previous_key:
            self._render_dirty = True
    ...
    def handle_normal_and_invincible_animation(self):
        ...
        if self.star_power == self.INVINCIBLE_POWERUP:
            ...
            if (self.player_animate_timer // self.INVINCIBILITY_ANIMATION_SPEED) % 2 == 0:
                self._current_visual_key = ("gold", self.current_direction)
            else:
                self._current_visual_key = ("normal", self.current_direction)
